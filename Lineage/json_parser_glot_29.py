...
#     walk(payload, "")
#     return results
def _extract_entries_with_context(payload: Any) -> List[Tuple[str, Optional[str], Dict[str, Any]]]:
    """
    Returns list of:
      (metadatakey, classname, view_sql_object)

    Works with ANY JSON shape.
    """
    results: List[Tuple[str, Optional[str], Dict[str, Any]]] = []

    def walk(node: Any, current_key: str = "", current_class: Optional[str] = None):
        if isinstance(node, dict):
            mk = current_key
            cls = current_class
