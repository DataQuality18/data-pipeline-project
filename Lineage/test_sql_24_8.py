...
def _resolve_source(qualifier: str, local_scope: Dict[str, Tuple], global_scope: Dict[str, Tuple]) -> Tuple[str, str, str]:
    ...
    # If column is unqualified and the query has a clear "primary" source table alias (common pattern),
    # default to that instead of marking ambiguous. This fixes CASE/DATE_FORMAT expressions where
    # columns are written without a table qualifier.
...
def _star_qualifier(star_node) -> str:
    """Return table qualifier for stars like CURRENT_RECORD.* if present."""
    try:
        if isinstance(star_node, exp.Column) and isinstance(star_node.this, exp.Star):
            return safe_name(star_node.table) or ""
        if isinstance(star_node, exp.Star):
            t = star_node.args.get("table")
            return safe_name(t) or ""
    except Exception:
        return ""
    return ""
...
def resolve_star(
    ...
    metadatakey,
    view_name,
    parent_subquery_alias: str | None = None,
    **_,
):
    """
    STAR handling:
      - Qualified star:  CURRENT_RECORD.*  -> resolve to that alias' base table (if available)
      - Unqualified star: SELECT * -> default to primary alias (TSR) if present, else single-scope, else ambiguous
      - If the resolved "table" is a placeholder (__DERIVED__/__SUBQUERY__), emit the alias itself as Table Name
        and add remark 'derived_table'.
    """
    ...
    # 1) Prefer explicit qualifier (CURRENT_RECORD.*)
    chosen = qualifier

    # 2) If unqualified, we can optionally use the enclosing/parent subquery alias if provided
    if not chosen and parent_subquery_alias and parent_subquery_alias in local_scope:
        chosen = parent_subquery_alias

    # 3) If still not chosen, default unqualified * to primary alias (TSR) if present
    if not chosen and "TSR" in local_scope:
        chosen = "TSR"

    # 4) Fallback to single source
    if not chosen and len(local_scope) == 1:
        chosen = next(iter(local_scope.keys()))

    if chosen:
        db, table, table_alias = _resolve_source(chosen, local_scope, global_scope)
        if (table or "").upper() in {"__DERIVED__", "__SUBQUERY__"} or not table:
            table = chosen
            table_alias = chosen
