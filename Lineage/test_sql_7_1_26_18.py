        ...
        while parent:
            ...
            if isinstance(subq, exp.Subquery) and subq.alias_or_name:
                ...
                if any(isinstance(e, exp.Star) for e in select.expressions):
                    ...
                    rows.append({
                        "Database Name": db or "",
                        "Table Name": base_table ,   # TSR_TS_DATA
...
def _emit_column_lineage(
    ...
    if not table:
        if effective_qualifier and effective_qualifier not in from_scope:
            # INVALID ALIAS → DO NOT invent table
...
def get_outer_derived_table(select: exp.Select) -> tuple[str | None, str | None]:
    """
        ...
        for col_text, col_alias, col_node in extract_select_list(select):
            ...
            if isinstance(col_node, exp.Column):
                ...
                if qualifier and qualifier in from_scope:
                    ...
                    if qualifier:
                        ...
                        table = ""
                        table_alias = qualifier   # capture the bad alias for reporting
                        db = ""
                    else:
                        # Unqualified column with multiple base tables → ambiguous
                        base_tables = [
            ...
            if derived_columns:
                for dcol in derived_columns:
                    ...
                    #  SINGLE-table fallback (existing behavior)
                    elif len(from_scope) == 1:
                        _, db, table, table_alias = next(iter(from_scope.values()))
                    elif qualifier:
                        remarks.append(REMARKS["INVALID_TABLE_ALIAS"])
                    ...
                    # NEW: outermost derived SELECT fallback 
                    else:
                        # If this CASE is in outermost SELECT, bind to derived dataset
                        parent = select.parent
                        while parent:
                            if isinstance(parent, exp.Select):
                                break
