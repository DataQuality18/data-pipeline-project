...
def build_from_scope_map(ast_root) -> Dict[str, Tuple]:
    ...
    Fix added:
    - Map JOIN subquery aliases like CURRENT_RECORD / PREVIOUS_RECORD
      so columns like CURRENT_RECORD.ID can resolve to underlying table.
    """
    from_map = {}

    # 1) All Tables (existing behavior)
    ...
    for alias in ast_root.find_all(exp.Alias):
        ...
        if isinstance(alias.this, exp.Table):
            ...
            # Keep old behavior, but at least store alias key
            from_map[key] = (alias, None, "__SUBQUERY__", key)

    # 3) ✅ NEW: JOIN Subquery alias mapping (CURRENT_RECORD / PREVIOUS_RECORD)
    # SQLGlot represents: (SELECT ...) CURRENT_RECORD as exp.Subquery(alias=...)
    for subq in ast_root.find_all(exp.Subquery):
        ...
        # Store alias -> underlying table mapping
        # Table Alias Name should be CURRENT_RECORD, Table Name should be base table name
