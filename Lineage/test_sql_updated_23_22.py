...
def resolve_star(select_node, star_node, from_scope, regulation, metadatakey, view_name):
    """Resolve SELECT * (and alias.*) into one or more lineage rows.

    Behavior:
      - alias.*  -> map to that alias entry (base table), include ALL_COLUMNS remark,
                   include DERIVED_TABLE if alias points to a derived subquery.
      - * with single source -> map to that one source
          - if source is derived: output Table Name = derived alias (per your requirement),
            and include DERIVED_TABLE remark.
      - * with multiple sources -> return one row per source (so you still get table + alias),
        and mark TABLE_AMBIGUOUS as well.
    """
    qualifier = ""
    # sqlglot can represent alias.* either as exp.Column(name="*", table="alias") or exp.Star(table="alias")
    if isinstance(star_node, exp.Column) and safe_name(star_node.this) == "*":
        qualifier = safe_name(star_node.table)
    elif isinstance(star_node, exp.Star):
        qualifier = safe_name(star_node.args.get("table"))

    def _mk(db: str, table: str, table_alias: str, extra_remarks=None):
        remarks = [REMARKS["ALL_COLUMNS"]]
        if extra_remarks:
            remarks.extend(extra_remarks)
        # keep your existing db remark style
    ...
    # 1) Qualified star: alias.*
    if qualifier:
        entry = from_scope.get(qualifier) or from_scope.get(qualifier.upper())
        if entry:
            extra = [REMARKS["DERIVED_TABLE"]] if getattr(entry, "is_derived", False) else []
            # For alias.*, keep the base table in Table Name, and alias in Table Alias Name
            return [_mk(entry.db, entry.table, entry.alias or qualifier, extra)]
        # unknown qualifier
        return [_mk("", "", qualifier, [REMARKS["TABLE_AMBIGUOUS"]])]

    # 2) Unqualified star: *
    uniq_entries = []
    # de-duplicate because we store both exact and upper-case keys
    seen_ids = set()
    for k, e in from_scope.items():
        if id(e) in seen_ids:
            continue
        seen_ids.add(id(e))
        uniq_entries.append((k, e))

    if len(uniq_entries) == 1:
        _, entry = uniq_entries[0]
        if getattr(entry, "is_derived", False):
            # Per requirement: outer query selecting from a derived table -> show derived alias as Table Name
            derived_name = entry.derived_name or entry.alias or ""
            return [_mk(entry.db, derived_name, entry.alias or derived_name, [REMARKS["DERIVED_TABLE"]])]
        return [_mk(entry.db, entry.table, entry.alias or entry.table, [])]

    if len(uniq_entries) > 1:
        rows = []
        for _, entry in uniq_entries:
            extra = [REMARKS["TABLE_AMBIGUOUS"]]
            if getattr(entry, "is_derived", False):
                extra.append(REMARKS["DERIVED_TABLE"])
            # For multi-table star, we still output per-source so you get table+alias.
            rows.append(_mk(entry.db, entry.table, entry.alias or entry.table, extra))
        return rows

    # 3) No from_scope
    return [_mk("", "", "", [REMARKS["TABLE_AMBIGUOUS"]])]
def resolve_column(
