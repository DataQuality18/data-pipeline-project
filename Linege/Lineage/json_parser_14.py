...
class JSONSQLParser:
    ...
    def _fetch_url_data(
        ...
        print(f"[DEBUG] _fetch_url_data called - URL: {url}, regulation: {regulation}, class_name: {class_name}")
        # Validate URL
    ...
    def parse_json_files(
        ...
        print(f"[DEBUG] parse_json_files called - regulation: {regulation}, class_name: {class_name}")
        # Convert view_names to tuple
        if isinstance(view_names, list):
            view_names = tuple(view_names)
    ...
    def _parse_single_json(
        ...
        print(f"[DEBUG] _parse_single_json - file: {json_file}, regulation: {regulation}, class_name: {class_name}")
        try:
    ...
    def _parse_single_data_object(
        ...
        print(f"[DEBUG] _parse_single_data_object - Processing record with regulation={data.get('regulation')}, name={data.get('name', '')}")
        # Early return for filter mismatches
        if (regulation and data.get('regulation') != regulation) or \
           (name and data.get('name', '') != name):
            print(f"[DEBUG] Skipping record - regulation or name mismatch")
            return []
        
        value = data.get('value', {})
        if class_name and value.get('classname', '') != class_name:
            print(f"[DEBUG] Skipping record - classname mismatch: expected={class_name}, got={value.get('classname', '')}")
            return []
        ...
        for query_obj in value.get('create_query', []):
            if not view_names or query_obj.get('view_name', '') in view_names:
                print(f"[DEBUG] Adding create_query with view_name={query_obj.get('view_name', '')}")
        ...
        if select_query := value.get('select_query'):
            if not view_names or select_query.get('view_name', '') in view_names:
                print(f"[DEBUG] Adding select_query with view_name={select_query.get('view_name', '')}")
