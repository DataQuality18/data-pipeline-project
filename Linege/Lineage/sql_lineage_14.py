...
class SQLLineageParser:
    ...
    def _create_error_record(self, query_key: str, error_message: str, view_name: str = '', metadatakey: str = '', regulation: str = '', class_name: str = '') -> Dict[str, str]:
    ...
    def _create_placeholder_record(self, query_key: str, view_name: str, metadatakey: str, regulation: str, class_name: str, remarks: str) -> Dict[str, str]:
    ...
    def parse_sql_files(self, sql_files: List[str], metadatakey: str = '', regulation: str = '', class_name: str = '') -> pd.DataFrame:
        ...
        for sql_file in sql_files:
            ...
            try:
                ...
                print(f"[DEBUG] parse_sql_files - regulation: {regulation}, metadatakey: {metadatakey}")
                
                query_type = self.detect_query_type(sql_content)
                
                if query_type in ['Mongo', 'Elastic']:
                    lineage_data = [self._create_non_sql_record(query_type, query_key, '', metadatakey, regulation, class_name)]
                    print(f"SUCCESS: Detected {query_type} Query")
                else:
                    lineage_data = self.parse_single_sql(sql_content, sql_file, query_key, '', metadatakey, regulation, class_name)
                ...
                query_key = Path(sql_file).stem.upper()
                all_lineage_data.append(self._create_error_record(query_key, f'failure_processing_file: {str(e)}', sql_file, metadatakey, regulation, class_name))
    ...
    def parse_query_dictionary(self, queries: Dict[str, str], metadata: Dict[str, Dict[str, str]] = None) -> pd.DataFrame:
        ...
        Args:
            ...
            metadata: Optional dictionary where key is query identifier and value contains
                     {'view_name': '...', 'metadatakey': '...', 'regulation': '...', 'class_name': '...'}
                    
        Returns:
            DataFrame with columns: Database Name, Table Name, Column Name, 
                                   Alias Name, Remarks, Status, View Name, Metadatakey, Regulation, Class Name
        ...
        Example:
            ...
            metadata = {
                'SQ1': {'view_name': 'CUSTOMER_VIEW', 'metadatakey': 'rhoo_controls_field_sql_query', 'regulation': 'rhoo', 'class_name': 'CustomerClass'},
                'SQ2': {'view_name': 'PRODUCT_VIEW', 'metadatakey': 'rhoo_controls_field_sql_query', 'regulation': 'rhoo', 'class_name': 'ProductClass'}
            }
        ...
        for query_key, sql_text in queries.items():
            ...
            class_name = query_metadata.get('class_name', '')
            print(f"[DEBUG] parse_query_dictionary - query_key: {query_key}, regulation: {regulation}, view_name: {view_name}")
            ...
            try:
                ...
                if query_type in ['Mongo', 'Elastic']:
                    ...
                    lineage_data = [self._create_non_sql_record(query_type, query_key, view_name, metadatakey, regulation, class_name)]
                    print(f"SUCCESS: Detected {query_type} Query for {query_key}")
                else:
                    # Handle SQL query
                    lineage_data = self.parse_single_sql(sql_text, query_key, query_key, view_name, metadatakey, regulation, class_name)
    ...
    def parse_single_sql(self, sql_content: str, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '', class_name: str = '') -> List[Dict[str, str]]:
        ...
        print(f"[DEBUG] parse_single_sql called - query_key: {query_key}, regulation: {regulation}")
        lineage_data = []
        ...
        if re.search(r'\bMAP\s*\(', sql_content, re.IGNORECASE):
            print(f"[DEBUG] MAP function detected in query: {query_key}")
            # Extract database and table from FROM clause
            from_match = re.search(r'\bFROM\s+([\w_.]+)', sql_content, re.IGNORECASE)
            if from_match:
                table_full = from_match.group(1)
                parts = table_full.split('.')
                database_name = parts[0] if len(parts) > 1 else ''
                table_name = parts[1] if len(parts) > 1 else parts[0]
                print(f"[DEBUG] MAP query - Extracted: Database={database_name}, Table={table_name}")
                ...
                return [{
                    ...
                    'metadatakey': metadatakey,
                    'regulation': regulation,
                    'remarks': 'Tech problem or spark complex SQL - map function',
                    'layer_order': 0
                }]
            else:
                return [self._create_placeholder_record(query_key, view_name, metadatakey, regulation, class_name, 'Tech problem or spark complex SQL - map function')]
        ...
        try:
            ...
            if not parsed:
                return [self._create_placeholder_record(query_key, view_name, metadatakey, regulation, class_name, 'failed_to_parse_sql')]
            ...
            for stmt in parsed:
                ...
                stmt_lineage = self.process_sql_statement(stmt, filename, query_key, view_name, metadatakey, regulation, class_name)
                lineage_data.extend(stmt_lineage)
                    
        except Exception as e:
            print(f"ERROR parsing SQL: {str(e)}")
            error_record = self._create_error_record(query_key, f'failure_sql_lineage_tech: {str(e)}', view_name, metadatakey, regulation, class_name)
            lineage_data.append(error_record)
        
        # If no lineage was extracted, return placeholder record to ensure metadatakey is preserved
        if not lineage_data:
            return [self._create_placeholder_record(query_key, view_name, metadatakey, regulation, class_name, 'no_columns_extracted_from_sql')]
            
        return lineage_data
    
    def process_sql_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '', class_name: str = '') -> List[Dict[str, str]]:
            ...
            if subquery_match:
                ...
                if inner_parsed:
                    for inner_stmt in inner_parsed:
                        inner_lineage = self.process_inner_statement(inner_stmt, filename, query_key, view_name, metadatakey, regulation, class_name)
                        ...
                        for entry in inner_lineage:
                            ...
                            if matched_outer:
                                ...
                                source_row['class_name'] = class_name
    ...
    def process_inner_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '', class_name: str = '') -> List[Dict[str, str]]:
    ...
    def parse_column_expression(self, col_expr: str) -> Dict[str, str]:
        ...
        # Check for * pattern (both table.* and plain *)
        if col_expr.strip() == '*':
            # Plain SELECT *
            return {
                'original_expression': col_expr,
        ...
        # Check if this is a hard-coded constant (string literal or number)
        is_constant = False
        if base_expr.startswith("'") and base_expr.endswith("'"):
            is_constant = True
        elif base_expr.startswith('"') and base_expr.endswith('"'):
            is_constant = True
        elif re.match(r'^\d+(\.\d+)?$', base_expr):
            is_constant = True
        
        # Skip hard-coded constants - return None to exclude from lineage
        if is_constant and not alias_name:
            return None
    ...
    def process_column_lineage(self, col_info: Dict, parent_tables: List, join_info: List, filename: str, level: int, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '', class_name: str = '') -> Dict[str, str]:
        ...
        if col_info.get('is_star'):
            # Check if it's SELECT * (without table prefix) or table.*
            if column_name == '*' and (not col_info.get('table_name') or table_name == 'unknown'):
                final_remark = 'all_columns_selected'
            else:
                final_remark = 'all_columns_selected'
