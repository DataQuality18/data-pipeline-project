...
class SQLLineageParser:
    ...
    def _create_non_sql_record(self, query_type: str, query_key: str, view_name: str, name: str = '', regulation: str = '') -> Dict[str, str]:
        """
        ...
        Args:
            ...
            name: Optional name metadata (top-level name field)
    ...
    def _create_placeholder_record(self, query_key: str, view_name: str, name: str, regulation: str, remarks: str) -> Dict[str, str]:
    ...
    def parse_query_dictionary(self, queries: Dict[str, str], metadata: Dict[str, Dict[str, str]] = None) -> pd.DataFrame:
        ...
        Args:
            ...
            metadata: Optional dictionary where key is query identifier and value contains
                     {'view_name': '...', 'name': '...', 'regulation': '...'}
                    
        Returns:
            DataFrame with columns: Database Name, Table Name, Column Name, 
                                   Alias Name, Remarks, Status, View Name, Name, Regulation
        ...
        Example:
            ...
            metadata = {
                'SQ1': {'view_name': 'CUSTOMER_VIEW', 'name': 'rhoo_controls_field_sql_query', 'regulation': 'rhoo'},
                'SQ2': {'view_name': 'PRODUCT_VIEW', 'name': 'rhoo_controls_field_sql_query', 'regulation': 'rhoo'}
            }
        ...
        for query_key, sql_text in queries.items():
            ...
            try:
                ...
                if query_type in ['Mongo', 'Elastic']:
                    # Handle non-SQL queries
                    lineage_data = [self._create_non_sql_record(query_type, query_key, view_name, name, regulation)]
                    print(f"SUCCESS: Detected {query_type} Query for {query_key}")
                else:
                    # Handle SQL query
                    lineage_data = self.parse_single_sql(sql_text, query_key, query_key, view_name, name, regulation)
                    # Add status to all records
                    for record in lineage_data:
                        record['status'] = 'success'
    ...
    def parse_single_sql(self, sql_content: str, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', name: str = '', regulation: str = '') -> List[Dict[str, str]]:
        ...
        if not sql_content.strip():
            return [self._create_placeholder_record(query_key, view_name, name, regulation, 'empty_sql_content')]
        ...
        try:
            ...
            if not parsed:
                return [self._create_placeholder_record(query_key, view_name, name, regulation, 'failed_to_parse_sql')]
            ...
            for stmt in parsed:
                ...
                stmt_lineage = self.process_sql_statement(stmt, filename, query_key, view_name, name, regulation)
            ...
            error_record.update({'name': name, 'regulation': regulation})
            lineage_data.append(error_record)
            
        return lineage_data
    
    def process_sql_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', name: str = '', regulation: str = '') -> List[Dict[str, str]]:
            ...
            if subquery_match:
                ...
                if inner_parsed:
                    for inner_stmt in inner_parsed:
                        inner_lineage = self.process_inner_statement(inner_stmt, filename, query_key, view_name, name, regulation)
        ...
        for col_info in select_columns:
            lineage_entry = self.process_column_lineage(col_info, main_tables, join_info, filename, 0, query_key, view_name, name, regulation)
            if lineage_entry:
                lineage_data.append(lineage_entry)
                
        return lineage_data
    
    def process_inner_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', name: str = '', regulation: str = '') -> List[Dict[str, str]]:
    ...
    def process_column_lineage(self, col_info: Dict, parent_tables: List, join_info: List, filename: str, level: int, query_key: str = 'UNKNOWN', view_name: str = '', name: str = '', regulation: str = '') -> Dict[str, str]:
    ...
    def create_lineage_dataframe(self, lineage_data: List[Dict[str, str]]) -> pd.DataFrame:
        ...
        column_order = [
            'Database Name', 'Table Name', 'Column Name',
            'Alias Name', 'Remarks', 'View Name', 'Name', 'Regulation'
        ]
