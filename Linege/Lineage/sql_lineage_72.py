...
class SQLLineageParser:
    ...
    def detect_query_type(self, query_text: str) -> str:
        ...
        # Check for Elastic query patterns first (before checking for {)
        # Elasticsearch queries contain specific keywords like "query", "bool", "match", "range", etc.
        elastic_keywords = ['elasticupdatedts', '"query"', '"bool"', '"must"', '"should"', '"match"', 
                           '"match_phrase"', '"range"', '"term"', '"terms"', '"aggs"', '"aggregations"',
                           '"filter"', '"bool"', 'get /', 'post /']
        
        if query_trimmed.startswith('[') or query_trimmed.startswith('GET ') or query_trimmed.startswith('POST '):
            return 'Elastic'
        
        # Check if query contains "elastic" keyword (case-insensitive)
        if 'elastic' in query_lower:
            return 'Elastic'
        
        # Check if JSON starts with { and contains Elasticsearch-specific patterns
        if query_trimmed.startswith('{'):
            # Count Elasticsearch-specific keywords
            elastic_count = sum(1 for keyword in elastic_keywords if keyword in query_lower)
            
            # If we find 2 or more Elasticsearch keywords, it's likely an Elastic query
            if elastic_count >= 2:
                return 'Elastic'
            
            # Check for MongoDB-specific patterns
            if 'db.' in query_trimmed[:100] or any(keyword in query_lower for keyword in ['$gt', '$lt', '$gte', '$lte', '$eq', '$ne', 'find(', 'aggregate(']):
                return 'Mongo'
            
            # If starts with { but no clear indicators, check structure
            # Elasticsearch queries typically have nested structure with "query" at top level
            if '"query"' in query_lower[:200] or query_lower.strip().startswith('{\n  "query"'):
                return 'Elastic'
            
            # Default JSON starting with { to Mongo
            return 'Mongo'
        
        # Default to SQL
        return 'SQL'
    
    def _create_non_sql_record(self, query_type: str, query_key: str, view_name: str, metadatakey: str = '', regulation: str = '') -> Dict[str, str]:
        """
        ...
        Args:
            ...
            metadatakey: Optional metadatakey metadata (top-level name field)
    ...
    def _create_error_record(self, query_key: str, error_message: str, view_name: str = '', metadatakey: str = '', regulation: str = '') -> Dict[str, str]:
        """
        ...
        Args:
            ...
            view_name: Name or path for display
            metadatakey: Optional metadatakey metadata
            regulation: Optional regulation metadata
    ...
    def _create_placeholder_record(self, query_key: str, view_name: str, metadatakey: str, regulation: str, remarks: str) -> Dict[str, str]:
    ...
    def parse_sql_files(self, sql_files: List[str], metadatakey: str = '', regulation: str = '') -> pd.DataFrame:
        """
        Parse multiple SQL files and extract column lineage information
        
        Args:
            sql_files: List of SQL file paths to parse
            metadatakey: Optional metadatakey for all files
            regulation: Optional regulation for all files
        ...
        for sql_file in sql_files:
            ...
            try:
                ...
                if query_type in ['Mongo', 'Elastic']:
                    # Handle non-SQL queries - pass metadatakey and regulation
                    lineage_data = [self._create_non_sql_record(query_type, query_key, sql_file, metadatakey, regulation)]
                    print(f"SUCCESS: Detected {query_type} Query")
                else:
                    # Handle SQL query - pass metadatakey and regulation
                    lineage_data = self.parse_single_sql(sql_content, sql_file, query_key, sql_file, metadatakey, regulation)
    ...
    def parse_query_dictionary(self, queries: Dict[str, str], metadata: Dict[str, Dict[str, str]] = None) -> pd.DataFrame:
        ...
        Args:
            ...
            metadata: Optional dictionary where key is query identifier and value contains
                     {'view_name': '...', 'metadatakey': '...', 'regulation': '...'}
                    
        Returns:
            DataFrame with columns: Database Name, Table Name, Column Name, 
                                   Alias Name, Remarks, Status, View Name, Metadatakey, Regulation
        ...
        Example:
            ...
            metadata = {
                'SQ1': {'view_name': 'CUSTOMER_VIEW', 'metadatakey': 'rhoo_controls_field_sql_query', 'regulation': 'rhoo'},
                'SQ2': {'view_name': 'PRODUCT_VIEW', 'metadatakey': 'rhoo_controls_field_sql_query', 'regulation': 'rhoo'}
            }
        ...
        for query_key, sql_text in queries.items():
            ...
            metadatakey = query_metadata.get('metadatakey', '')
            ...
            try:
                ...
                if query_type in ['Mongo', 'Elastic']:
                    ...
                    lineage_data = [self._create_non_sql_record(query_type, query_key, view_name, metadatakey, regulation)]
                    print(f"SUCCESS: Detected {query_type} Query for {query_key}")
                else:
                    # Handle SQL query
                    lineage_data = self.parse_single_sql(sql_text, query_key, query_key, view_name, metadatakey, regulation)
                    # Add status to all records
                    for record in lineage_data:
                        record['status'] = 'success'
    ...
    def parse_single_sql(self, sql_content: str, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '') -> List[Dict[str, str]]:
        ...
        if not sql_content.strip():
            return [self._create_placeholder_record(query_key, view_name, metadatakey, regulation, 'empty_sql_content')]
        ...
        try:
            ...
            if not parsed:
                return [self._create_placeholder_record(query_key, view_name, metadatakey, regulation, 'failed_to_parse_sql')]
            ...
            for stmt in parsed:
                ...
                stmt_lineage = self.process_sql_statement(stmt, filename, query_key, view_name, metadatakey, regulation)
                lineage_data.extend(stmt_lineage)
                    
        except Exception as e:
            print(f"ERROR parsing SQL: {str(e)}")
            error_record = self._create_error_record(query_key, f'failure_sql_lineage_tech: {str(e)}', view_name, metadatakey, regulation)
            lineage_data.append(error_record)
        
        # If no lineage was extracted, return placeholder record to ensure metadatakey is preserved
        if not lineage_data:
            return [self._create_placeholder_record(query_key, view_name, metadatakey, regulation, 'no_columns_extracted_from_sql')]
            
        return lineage_data
    
    def process_sql_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '') -> List[Dict[str, str]]:
            ...
            if subquery_match:
                ...
                if inner_parsed:
                    for inner_stmt in inner_parsed:
                        inner_lineage = self.process_inner_statement(inner_stmt, filename, query_key, view_name, metadatakey, regulation)
                        ...
                        for entry in inner_lineage:
                            ...
                            if matched_outer:
                                ...
                                source_row['metadatakey'] = metadatakey
        ...
        for col_info in select_columns:
            lineage_entry = self.process_column_lineage(col_info, main_tables, join_info, filename, 0, query_key, view_name, metadatakey, regulation)
            if lineage_entry:
                lineage_data.append(lineage_entry)
                
        return lineage_data
    
    def process_inner_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '') -> List[Dict[str, str]]:
    ...
    def get_remarks(self, sql_content: str) -> str:
        ...
        sql_trimmed = sql_content.strip()
        
        if sql_trimmed.startswith('['):
            return "ignored_elastic_query"
        
        if sql_trimmed.startswith('{'):
            return "ignored_mongo_query"
    ...
    def extract_select_columns(self, stmt) -> List[Dict[str, str]]:
        ...
        for col_expr in column_expressions:
            ...
            if col_info:
                # Skip table.column extraction for window functions (they're already handled)
                original_expr_upper = col_info.get('original_expression', '').upper()
                is_window_function = 'OVER(' in original_expr_upper or 'OVER (' in original_expr_upper
                
                # For complex derived expressions (excluding window functions), extract all table.column references
                if col_info.get('is_derived') and not is_window_function and col_info.get('column_name') in ['derived_expression', col_info.get('alias_name')]:
    ...
    def process_column_lineage(self, col_info: Dict, parent_tables: List, join_info: List, filename: str, level: int, query_key: str = 'UNKNOWN', view_name: str = '', metadatakey: str = '', regulation: str = '') -> Dict[str, str]:
        ...
        if col_info.get('is_derived'):
            ...
            # Check for MAP function (Spark complex SQL)
            elif 'MAP(' in original_expr:
                remarks = "Tech problem or spark complex SQL map"
            else:
    ...
    def _determine_column_remarks(self, original_expr: str, database_name: str, table_name: str, total_tables: int) -> str:
        ...
        # Check for MAP function (Spark complex SQL)
        if 'MAP(' in expr_upper:
            return "Tech problem or spark complex SQL map"
        ...
        if not table_name or table_name == 'unknown':
            return "table_name_ambigious" if total_tables > 1 else ""
    ...
    def create_lineage_dataframe(self, lineage_data: List[Dict[str, str]]) -> pd.DataFrame:
        ...
        column_order = [
            'Database Name', 'Table Name', 'Column Name',
            'Alias Name', 'Remarks', 'View Name', 'Metadatakey', 'Regulation'
        ]
