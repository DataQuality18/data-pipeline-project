...
class JSONSQLParser:
    ...
    def parse_from_url(
        ...
        try:
            ...
            # Check if data has metadataList wrapper
            if 'metadataList' in data:
                metadata_list = data['metadataList']
                print(f"Received metadataList with {len(metadata_list)} records")
                for idx, item in enumerate(metadata_list, 1):
                    if idx % 100 == 0:
                        print(f"  Processing record {idx}/{len(metadata_list)}...")
    ...
    def _parse_single_json(
        ...
        try:
            ...
            # Check if data has metadataList wrapper
            if 'metadataList' in data:
                for idx, item in enumerate(data['metadataList'], 1):
                    item_results = self._parse_single_data_object(
                        item, 
                        f"{file_name}_record_{idx}",
                        regulation, 
                        classname, 
                        view_names
                    )
                ...
                # Single object without wrapper
                return self._parse_single_data_object(data, file_name, regulation, classname, view_names)
    ...
    def _parse_single_data_object(
        ...
        for query_obj in value.get('create_query', []):
            ...
            if not view_names or view_name in view_names:
                ...
                encoded_sql = query_obj.get('sql_query', '')
                try:
                    decoded_bytes = base64.b64decode(encoded_sql)
                    decoded_sql = decoded_bytes.decode('utf-8')
                except Exception:
                    decoded_sql = encoded_sql  # Use as-is if not base64 encoded
                
                results.append({
                    **common_metadata,
                    'view_name': view_name,
                    'sql_query': decoded_sql
                })
        ...
        if select_query := value.get('select_query'):
            ...
            if not view_names or view_name in view_names:
                # Decode base64 encoded SQL query
                encoded_sql = select_query.get('sql_query', '')
                try:
                    decoded_bytes = base64.b64decode(encoded_sql)
                    decoded_sql = decoded_bytes.decode('utf-8')
                except Exception:
...
def api_parse_from_url(request: URLParseRequest):
    ...
    **Example Request:**
    ```json
    {
      ...
      "view_names": ["APP_REGHUB_RHOO_CONTROLS_FIELD_GRU", "APP_REGHUB_RHOO_CONTROLS_FIELD_REG"]
    }
    ```
    
    **Note:** view_names will be converted to tuple internally.
