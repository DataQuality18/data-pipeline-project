...
class SQLLineageParser:
    ...
    def process_sql_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', classname: str = '', regulation: str = '') -> List[Dict[str, str]]:
            ...
            if subquery_match:
                ...
                if inner_parsed:
                    for inner_stmt in inner_parsed:
                        inner_lineage = self.process_inner_statement(inner_stmt, filename, query_key, view_name, classname, regulation)
        ...
        for col_info in select_columns:
            lineage_entry = self.process_column_lineage(col_info, main_tables, join_info, filename, 0, query_key, view_name, classname, regulation)
            if lineage_entry:
                lineage_data.append(lineage_entry)
                
        return lineage_data
    
    def process_inner_statement(self, stmt, filename: str, query_key: str = 'UNKNOWN', view_name: str = '', classname: str = '', regulation: str = '') -> List[Dict[str, str]]:
